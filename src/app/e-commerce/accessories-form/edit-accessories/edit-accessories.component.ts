...
export class EditAccessoriesComponent implements OnInit {
  ...
  getProductDetails() {
    let data = { _id: this.productId }
    this._generalService.productDetail(data).subscribe(
      (response) => {
        if (response['code'] == genralConfig.statusCode.ok) {
          ...
          this.formOne.patchValue({ color: response['data'].color })
          this.formTwo.patchValue(response['data'])
  ...
  getSubCategoryList(id) {
    let data = { sortValue: 'category_name', sortOrder: 1, parentCategoryId: id }
  ...
  getBrandList() {
    ...
    this._generalService.getBrandList(data).subscribe((response) => {
      if (response['code'] == genralConfig.statusCode.ok) this.brandList = response['data']
    })
  }
