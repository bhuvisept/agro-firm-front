...
export class PaymentPageComponent implements OnInit {
  ...
  subsPlanList() {
    ...
    this.service.subsPlanList({ userId: this.userId }).subscribe((res) => {
      if (res['code'] == 200) {
        ...
        if (res['promoData'] && res['promoData'].appliedCode) {
          this.finalPrice = res['GrandTotal']
        } else {
          this.finalPrice = this.userPlanData['finalPrice']
        }
  ...
  customPlanData() {
    this.service.customPlanDetails({ token: this.userData.userInfo.paymentToken }).subscribe((res) => {
      if (res['code'] == 200) {
        this.finalPrice = res['data'].planPrice
      }
    })
  }
